...
public class HTMLGenerator {
		...
		for (File f : template.listFiles()) {
			...
			while (generate != -1) {
				...
				if (genType.equals("effects") || isDocsPage) {
					for (Iterator<SyntaxElementInfo<? extends Effect>> it = sortedAnnotatedIterator(((Collection<SyntaxElementInfo<? extends Effect>>) (Collection<?>) Skript.getEffects()).iterator()); it.hasNext(); ) {
					...
					for (Iterator<SyntaxElementInfo<? extends Section>> it = sortedAnnotatedIterator(((Collection<SyntaxElementInfo<? extends Section>>) (Collection<?>) Skript.getSections()).iterator()); it.hasNext(); ) {
				...
				if (genType.equals("conditions") || isDocsPage) {
					for (Iterator<SyntaxElementInfo<? extends Condition>> it = sortedAnnotatedIterator(((Collection<SyntaxElementInfo<? extends Condition>>) (Collection<?>) Skript.getConditions()).iterator()); it.hasNext(); ) {
	...
	private String generateAnnotated(String descTemp, SyntaxElementInfo<?> info, @Nullable String page, String type) {
		...
		String ID = docId != null ? (docId != null ? docId.value() : null) : info.getElementClass().getSimpleName();
		// Fix duplicated IDs
		if (page != null) {
