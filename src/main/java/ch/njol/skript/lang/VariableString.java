...
public class VariableString implements ch.njol.skript.lang.Expression<String> {
	...
	public String toString(@Nullable Event event) {
		...
		for (Object object : string) {
			if (object instanceof Expression<?>) {
				Object[] objects = ((Expression<?>) object).getArray(new BukkitTriggerContext(event, event.getEventName()));
	...
	public String toString(@Nullable Event event, boolean debug) {
		...
		for (Object object : string) {
			if (object instanceof Expression) {
				builder.append("%").append(((Expression<?>) object).toString(event != null ? new BukkitTriggerContext(event, event.getEventName()) : TriggerContext.dummy(), debug)).append("%");
			} else {
				builder.append(object);
			}
		}
